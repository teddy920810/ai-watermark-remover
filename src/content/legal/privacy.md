...
For object removal, WatermarkGemini sends short-lived signed links for the uploaded image and the black-and-white selection mask to Replicate, which runs the LaMa inpainting model. The same Replicate API retention rules apply. WatermarkGemini does not send your Google profile information to Replicate for image processing.
...
WatermarkGemini relies on service providers for Google authentication, website hosting, managed databases, private object storage, analytics, and image processing, including Replicate for the background-removal and object-removal features. They may process data only to provide those services under their applicable terms and safeguards.
