...
const slug = 'how-to-remove-watermark-tiktok';
const imageSlug = 'how-to-remove-tiktok-watermark';
const imageHashes = [
...
describe('operations TikTok watermark article', () => {
  it('permanently redirects the retired article URL without publishing duplicate content', () => {
    const config = JSON.parse(fs.readFileSync('vercel.json', 'utf8'));
    expect(config.redirects).toContainEqual({
      source: '/blog/how-to-remove-tiktok-watermark',
      destination: '/blog/how-to-remove-watermark-tiktok',
      permanent: true,
    });
    expect(fs.existsSync('src/content/blog/how-to-remove-tiktok-watermark.md')).toBe(false);
  });
  ...
  it('publishes the supplied metadata and retains the video versus still-image distinction', () => {
    ...
    expect(data.coverImage).toBe(`/uploads/${imageSlug}-figure-1.png`);
    ...
    for (let index = 1; index <= 4; index++) expect(source).toContain(`/uploads/${imageSlug}-figure-${index}.png`);
  });

  it('preserves all four original Word images byte for byte', () => {
    for (const [index, hash] of imageHashes.entries()) {
      const image = fs.readFileSync(path.join(process.cwd(), 'public/uploads', `${imageSlug}-figure-${index + 1}.png`));
