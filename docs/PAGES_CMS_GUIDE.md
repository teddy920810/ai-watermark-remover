...
在 **Sitemap 设置 / Sitemap settings** 中可以维护公共页面 `lastmod`，以及不同页面类型的 `changefreq` 和 `priority`。需要提高单个真实页面的优先级时，在“单个页面覆盖规则”中填写以 `/` 开头和结尾的完整路径。新增或删除内容后，等待部署完成并访问 `https://www.watermarkgemini.com/sitemap.xml` 检查。
